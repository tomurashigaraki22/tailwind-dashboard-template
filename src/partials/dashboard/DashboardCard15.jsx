...
function DashboardCard15() {
  ...
  useEffect(() => {
    const fetchSecrets = async () => {
      try {
        const response = await fetch('https://skeletonserver.onrender.com/ads', {
          method: 'GET',
        });
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
        ...
        formdata.append("ad_id", selectedSecret[1]) 
      const response = await fetch('https://skeletonserver.onrender.com/delete_ad', {
        method: 'POST',
        body: formdata
      });

      if (response.ok) {
        setUsers(users.filter(user => user[1] !== selectedSecret[1]));
        setError("");
      } else {
  ...
  return (
    <div className="col-span-full xl:col-span-12 bg-white dark:bg-gray-800 shadow-sm rounded-xl w-full">
      <header className="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60">
        <h2 className="font-semibold text-gray-800 dark:text-gray-100">All Ad Requests</h2>
      </header>
      <div className="p-3">
        {loading ? (
          <div className='px-2'>Loading all requests...</div>
        ) : error ? (
            ...
            <div style={{ height: '400px', overflowY: 'auto' }}>
              <table className="table-auto w-full dark:text-gray-300">
                <thead className="text-xs uppercase text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-700 dark:bg-opacity-50 rounded-sm">
                  <tr>
                    <th className="p-2">
                      <div className="font-semibold text-left">Ads ID</div>
                    </th>
                    <th className="p-2">
                      <div className="font-semibold text-center">Ads Title and Body</div>
                    </th>
                    <th className="p-2">
                      <div className="font-semibold text-center">Ads Email</div>
                    </th>
                    <th className="p-2">
                    ...
                    <tr key={index}>
                      <td className="p-2">
                        <div className="flex items-center space-x-3">
                          ...
                          <div className="text-gray-800 dark:text-gray-100">{user[1]}</div>
                        </div>
                      </td>
                      <td className="p-2 px-10">
                        <div className="text-center text-green-500">{`${user[3]} + ${user[4]}`}</div>
                      </td>
                      <td className="p-2 px-5">
                        <div className="text-center">{user[2]}</div>
                      </td>
